...
原生群聊总结助手。它可同时监听微信与 Discord 的群聊指令，按指定时间范围读取消息，调用兼容 OpenAI Chat Completions 的模型生成中文总结，并可选生成配图后发回原群。
...
- 5xx 指数退避重试、429 串行重试队列、多 API Key 并发控制、失败原因与请求 trace 脱敏落盘，以及可按任务生成诊断包。
...
同时启用两个平台时，在 GUI“接入平台”页勾选“微信”和“Discord”，或写入：
...
旧配置只设置 `kind = "wx"` 或 `kind = "discord"` 时仍按原来的单平台方式运行。定时任务、周报与 Outbox 会依照群聊/频道的配置归属自动路由到正确平台。
...
- “仅允许指定用户触发”默认关闭。开启后，只有 `listen.allowed_users` 中的用户 ID 能触发总结；房间能力或策略模板可单独覆盖此规则。
- “默认总结详细度”可设为 `concise`、`standard` 或 `detailed`。手动指令增加 `预览`、`preview` 或 `dry-run` 时，只生成文字总结并保存在任务中心，不发送到群。
- 每群每日预算默认关闭；开启后可分别限制总结次数、图片投递次数和媒体处理量，`0` 表示不限制。房间显式值优先于模板，再优先于全局值。
- 任务中心支持取消、按原时间范围重试和生成按任务诊断包。诊断包包含脱敏配置、任务阶段、来源索引、投递状态、供应商健康记录和日志尾部，不保存原始聊天正文或媒体文件。
- 所有群消息会先进入持久化 Outbox。进程在发送途中中断时，该投递会标记为“待确认”而不会自动重发，避免消息可能已送达时刷屏；可在任务中心手动确认重试或取消。
- LLM 主供应商连续失败三次后会熔断五分钟。后续任务会跳过主供应商，直接按 `fallbacks` 顺序使用备用供应商；认证和参数类确定性 4xx 不应作为供应商故障配置到备用链中。
...
- GUI 不回显 API Key；不要将真实密钥提交到 Git。文本脱敏默认关闭，需要时可在 TOML 中显式开启 `privacy.redact_enabled`。
