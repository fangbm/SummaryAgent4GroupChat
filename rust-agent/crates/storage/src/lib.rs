...
impl SqliteStateStore {
    ...
    pub fn tasks(&self, limit: usize, room_id: Option<&str>, state: Option<TaskState>) -> Result<Vec<TaskRecord>, StorageError> {
        let conn = self.conn.lock().map_err(|_| StorageError::Poisoned)?;
        ...
        let rows = statement.query_map(params![room_id, state.map(TaskState::as_str), limit.max(1) as i64], row_to_task)?;
    ...
    pub fn update_task(&self, id: &str, state: TaskState, stage: &str, summary: Option<&str>, error: Option<&str>, message_count: u64, media_count: u64) -> Result<(), StorageError> {
        let now = Utc::now().to_rfc3339();
        let conn = self.conn.lock().map_err(|_| StorageError::Poisoned)?;
        let finished = matches!(state, TaskState::Succeeded | TaskState::Failed | TaskState::Cancelled);
    ...
    pub fn add_source_references(&self, references: &[SourceReference]) -> Result<(), StorageError> {
    ...
    pub fn source_references(&self, task_id: &str) -> Result<Vec<SourceReference>, StorageError> {
        ...
        let rows = statement.query_map(params![task_id], |row| Ok(SourceReference { task_id: row.get(0)?, point_index: row.get::<_, i64>(1)? as u32, source_id: row.get(2)?, occurred_at: parse_time(row.get::<_, String>(3)?)?, sender_label: row.get(4)?, message_index: row.get::<_, i64>(5)? as u32 }))?;
        rows.collect::<Result<Vec<_>, _>>().map_err(StorageError::from)
    }

    pub fn enqueue_delivery(&self, task_id: Option<&str>, room_id: &str, kind: &str, payload: &str) -> Result<DeliveryRecord, StorageError> {
        let now = Utc::now();
        let record = DeliveryRecord { id: Uuid::new_v4().to_string(), task_id: task_id.map(ToOwned::to_owned), room_id: room_id.to_string(), kind: kind.to_string(), payload: payload.to_string(), state: DeliveryState::Pending, attempts: 0, next_attempt_at: now, created_at: now, updated_at: now, error: None };
        let conn = self.conn.lock().map_err(|_| StorageError::Poisoned)?;
        conn.execute("insert into outbox_delivery(id, task_id, room_id, kind, payload, state, attempts, next_attempt_at, created_at, updated_at) values(?1,?2,?3,?4,?5,?6,0,?7,?7,?7)", params![record.id, record.task_id, record.room_id, record.kind, record.payload, record.state.as_str(), record.next_attempt_at.to_rfc3339()])?;
        Ok(record)
    }

    pub fn due_deliveries(&self, limit: usize) -> Result<Vec<DeliveryRecord>, StorageError> {
        let conn = self.conn.lock().map_err(|_| StorageError::Poisoned)?;
        let mut statement = conn.prepare("select id, task_id, room_id, kind, payload, state, attempts, next_attempt_at, created_at, updated_at, error from outbox_delivery where state='pending' and next_attempt_at<=?1 order by created_at limit ?2")?;
        let rows = statement.query_map(params![Utc::now().to_rfc3339(), limit.max(1) as i64], row_to_delivery)?;
        rows.collect::<Result<Vec<_>, _>>().map_err(StorageError::from)
    }

    pub fn deliveries(&self, limit: usize) -> Result<Vec<DeliveryRecord>, StorageError> {
        let conn = self.conn.lock().map_err(|_| StorageError::Poisoned)?;
        let mut statement = conn.prepare("select id, task_id, room_id, kind, payload, state, attempts, next_attempt_at, created_at, updated_at, error from outbox_delivery order by created_at desc limit ?1")?;
        let rows = statement.query_map(params![limit.max(1) as i64], row_to_delivery)?;
        rows.collect::<Result<Vec<_>, _>>().map_err(StorageError::from)
    }

    pub fn update_delivery(&self, id: &str, state: DeliveryState, attempts: u32, next_attempt_at: DateTime<Utc>, error: Option<&str>) -> Result<(), StorageError> {
        let conn = self.conn.lock().map_err(|_| StorageError::Poisoned)?;
        conn.execute("update outbox_delivery set state=?2, attempts=?3, next_attempt_at=?4, updated_at=?5, error=?6 where id=?1", params![id, state.as_str(), attempts as i64, next_attempt_at.to_rfc3339(), Utc::now().to_rfc3339(), error])?;
        Ok(())
    }
    ...
    pub fn update_provider_health(&self, capability: &str, provider_key: &str, consecutive_failures: u32, circuit_open_until: Option<DateTime<Utc>>, last_error: Option<&str>) -> Result<(), StorageError> {
        let conn = self.conn.lock().map_err(|_| StorageError::Poisoned)?;
    ...
    pub fn provider_health(&self) -> Result<Vec<ProviderHealth>, StorageError> {
        ...
        let rows = statement.query_map([], |row| Ok(ProviderHealth { capability: row.get(0)?, provider_key: row.get(1)?, consecutive_failures: row.get::<_, i64>(2)? as u32, circuit_open_until: row.get::<_, Option<String>>(3)?.map(parse_time).transpose()?, last_error: row.get(4)?, updated_at: parse_time(row.get(5)?)? }))?;
        rows.collect::<Result<Vec<_>, _>>().map_err(StorageError::from)
    }

    pub fn create_weekly_metric(&self, group_name: &str, room_id: &str) -> Result<String, StorageError> {
    ...
    pub fn update_weekly_metric(&self, id: &str, state: &str, summary: Option<&str>, chart_path: Option<&str>, error: Option<&str>) -> Result<(), StorageError> {
        let conn = self.conn.lock().map_err(|_| StorageError::Poisoned)?;
        conn.execute("update weekly_report set state=?2, summary=?3, chart_path=?4, error=?5 where id=?1", params![id, state, summary, chart_path, error])?;
        Ok(())
    }
    ...
    pub fn weekly_metrics(&self, limit: usize) -> Result<Vec<WeeklyMetric>, StorageError> {
        ...
        let rows = statement.query_map(params![limit.max(1) as i64], |row| Ok(WeeklyMetric { report_id: row.get(0)?, group_name: row.get(1)?, room_id: row.get(2)?, state: row.get(3)?, created_at: parse_time(row.get(4)?)?, summary: row.get(5)?, chart_path: row.get(6)?, error: row.get(7)? }))?;
        rows.collect::<Result<Vec<_>, _>>().map_err(StorageError::from)
    }
    ...
    pub fn cleanup_operational_data(&self, retention_days: i64) -> Result<(), StorageError> {
        ...
        conn.execute("delete from outbox_delivery where created_at < ?1", params![cutoff])?;
        conn.execute("delete from summary_task where created_at < ?1", params![cutoff])?;
        conn.execute("delete from weekly_report where created_at < ?1", params![cutoff])?;
        Ok(())
    }
...
fn row_to_task(row: &rusqlite::Row<'_>) -> Result<TaskRecord, rusqlite::Error> {
    Ok(TaskRecord {
        id: row.get(0)?, room_id: row.get(1)?, source: row.get(2)?, state: TaskState::parse(&row.get::<_, String>(3)?), stage: row.get(4)?,
        since: parse_time(row.get(5)?)?, until: parse_time(row.get(6)?)?, created_at: parse_time(row.get(7)?)?,
        started_at: row.get::<_, Option<String>>(8)?.map(parse_time).transpose()?, completed_at: row.get::<_, Option<String>>(9)?.map(parse_time).transpose()?,
        summary: row.get(10)?, error: row.get(11)?, retry_of: row.get(12)?, config_revision: row.get(13)?, message_count: row.get::<_, i64>(14)? as u64, media_count: row.get::<_, i64>(15)? as u64,
    })
}

fn row_to_delivery(row: &rusqlite::Row<'_>) -> Result<DeliveryRecord, rusqlite::Error> {
    Ok(DeliveryRecord {
        id: row.get(0)?, task_id: row.get(1)?, room_id: row.get(2)?, kind: row.get(3)?, payload: row.get(4)?, state: DeliveryState::parse(&row.get::<_, String>(5)?),
        attempts: row.get::<_, i64>(6)? as u32, next_attempt_at: parse_time(row.get(7)?)?, created_at: parse_time(row.get(8)?)?, updated_at: parse_time(row.get(9)?)?, error: row.get(10)?,
    })
}
...
mod tests {
    ...
    fn persists_task_sources_and_delivery() {
        ...
        store.update_task(&task.id, TaskState::Running, "history", None, None, 4, 1).unwrap();
        store.add_source_references(&[SourceReference { task_id: task.id.clone(), point_index: 1, source_id: "m-1".into(), occurred_at: now, sender_label: "用户 #1".into(), message_index: 3 }]).unwrap();
        let delivery = store.enqueue_delivery(Some(&task.id), "room-a", "text", "summary").unwrap();
        assert_eq!(store.tasks(10, Some("room-a"), None).unwrap().len(), 1);
        assert_eq!(store.source_references(&task.id).unwrap()[0].source_id, "m-1");
        assert_eq!(store.due_deliveries(10).unwrap()[0].id, delivery.id);
        store.update_delivery(&delivery.id, DeliveryState::Delivered, 1, now, None).unwrap();
        assert_eq!(store.deliveries(10).unwrap()[0].state, DeliveryState::Delivered);
    }

    #[test]
    fn provider_health_round_trips() {
        let store = SqliteStateStore::in_memory().unwrap();
        store.update_provider_health("llm", "primary", 3, Some(Utc::now()), Some("timeout")).unwrap();
