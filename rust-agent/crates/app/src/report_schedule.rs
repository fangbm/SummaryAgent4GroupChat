...
pub(crate) fn aggregate(
    tasks: impl IntoIterator<Item = impl std::borrow::Borrow<TaskRecord>>,
) -> WeeklyStats {
    tasks
        .into_iter()
        .fold(WeeklyStats::default(), |mut stats, task| {
...
pub(crate) fn next_run_after(
    ...
    let candidate = Local
        .from_local_datetime(
            &(local_now.date_naive() + Duration::days(days as i64)).and_hms_opt(
                group.local_hour,
                group.local_minute,
                0,
            )?,
        ...
        .single()?;
    Some(if candidate <= local_now {
        (candidate + Duration::days(7)).with_timezone(&Utc)
    } else {
