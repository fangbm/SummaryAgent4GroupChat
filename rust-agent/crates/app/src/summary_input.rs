...
pub(crate) fn prepare(config: &AgentConfig, history: Vec<PlatformHistoryMessage>) -> PreparedSummaryInput {
    let messages = history.into_iter().map(history_to_chat_message).collect::<Vec<_>>();
    ...
    PreparedSummaryInput { messages, total_messages: formatted.total_messages, llm_input, privacy }
}

pub(crate) fn persist_sources(task: &OperationalTask, messages: &[ChatMessage]) {
    let references = messages.iter().enumerate().map(|(index, message)| SourceReference {
        task_id: task.id.clone(), point_index: 0, source_id: format!("m-{}", index + 1),
