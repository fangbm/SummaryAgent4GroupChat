...
listening, text sends, and image/file sends. History is supplied by a separately
installed external command because wx4py's UIA history reader cannot reliably
...
default Windows provider is `wx4py` plus an externally configured history command.
...
- This is retained only as historical compatibility context. The default branch uses an externally configured history command while `wx4py` handles live UI automation.
